...
BLE positioning is zone-first for the FlyCare demo. The 12 registered beacon MACs are grouped into six airport zones (Check-in, Security Check, Toilet, Customer Services, Gate 10, and Gate 11) before XY is calculated, so duplicated MACs in one area no longer double the area's weighting. The raw weighted XY is still logged, then the displayed/stable XY is corrected toward the confirmed zone center. The small-field demo profile requires three consecutive candidate scans before switching zones, compares the best zone against the second-best zone for RSSI/score margin, and only applies strongest-zone snap after the stable zone is confirmed. While a candidate zone is pending, the marker is locked to the last stable XY instead of chasing single-scan RSSI drift. Small display movements under `DISPLAY_POSITION_REDRAW_THRESHOLD_METERS` do not repaint the map.
...
Arrival is also debounced with `FLIGHT_ARRIVAL_CONFIRMATIONS`: the same target must remain valid for consecutive position updates before the popup is shown. In the current 6-second BLE scan cadence this favors accuracy over instant feedback and avoids one-scan RSSI drift between Customer Services and Gate 11. The current small-field profile uses three confirmations, so a correct arrival can take roughly 12-18 seconds while false Gate 10/Gate 11 popups are suppressed.
