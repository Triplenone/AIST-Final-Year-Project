...
Alert sounds are queued before or alongside TTS for SOS, delay, boarding, final call, cancellation, on-time updates, and arrival. Gate Change intentionally skips alert audio/TTS and uses the large popup plus short vibration, because the final-demo watch must avoid the SD_MMC alert-file lookup path that can abort/reset during retained flight downlinks. Missing alert sound files are logged and the firmware safely continues with TTS for the other alert types.
