...
BLE positioning is zone-first for the FlyCare demo. The 12 registered beacon MACs are grouped into six airport zones (Check-in, Security Check, Toilet, Customer Services, Gate 10, and Gate 11) before XY is calculated, so duplicated MACs in one area no longer double the area's weighting. The raw weighted XY is still logged, then the displayed/stable XY is lightly corrected toward the confirmed zone center. A candidate zone must win consecutive scans before it replaces the stable zone, and small display movements under `DISPLAY_POSITION_REDRAW_THRESHOLD_METERS` do not repaint the map. This keeps the UI smooth without hiding the raw BLE evidence needed for tuning.
...
Arrival is also debounced with `FLIGHT_ARRIVAL_CONFIRMATIONS`: the same target must remain valid for consecutive position updates before the popup is shown. In the current 6-second BLE scan cadence this favors accuracy over instant feedback and avoids one-scan RSSI drift between Customer Services and Gate 11.
