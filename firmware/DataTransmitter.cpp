...
void DataTransmitter::triggerDoorOpen(const String& door_id) {
    ...
    json += "\"distance\":1.2,";
    json += "\"trigger_time\":" + String(getCurrentTimestamp()) + ",";
    json += "\"action\":\"unlock\"";
    json += "}]";
    json += "}";
...
void DataTransmitter::triggerLightControl(bool turn_on) {
    ...
    json += "\"last_movement\":" + String(getCurrentTimestamp());
    json += "}";
    json += "}";
...
void DataTransmitter::transmitLocation() {
    if (beacon_count <= 0 || location_quality == "unknown") {
        Serial.printf("[MQTT_PUB] location skipped: invalid location quality=%s beacons=%d\n",
                      location_quality.c_str(), beacon_count);
        return;
    }
...
void DataTransmitter::checkNightMovement(float current_x, float current_y, 
    ...
    static unsigned long last_check = getCurrentTimestamp();
    ...
    // 检查是否在CA信标1米范围内
    String normalized_beacon_mac = current_beacon_mac;
    normalized_beacon_mac.toUpperCase();
    bool customer_services_beacon =
        normalized_beacon_mac == "20:A7:16:60:F7:CA" ||
        normalized_beacon_mac == "20:A7:16:61:02:42";
    bool in_range = (customer_services_beacon && beacon_distance <= 1.0);
    
    if (in_range && !was_in_range) {
        enter_range_time = getCurrentTimestamp();
        last_x = current_x;
        last_y = current_y;
    ...
    if (in_range) {
        ...
        float time_in_range = now - enter_range_time;
        
        if (time_in_range >= 3.0) {
            float time_diff = now - last_check;
    ...
    last_movement_time = getCurrentTimestamp();
}
...
void DataTransmitter::updateHeartRate(int bpm, float confidence) {
    ...
    heart_rate.timestamp = getCurrentTimestamp();
...
void DataTransmitter::updateSpO2(int percentage, float confidence) {
    ...
    spo2.timestamp = getCurrentTimestamp();
    spo2.valid = (percentage >= 70 && percentage <= 100 && confidence > 0.0);
...
String DataTransmitter::getDoorEventsJSON() {
    ...
    for (int i = 0; i < door_count; i++) {
        ...
        json += "\"trigger_time\":" + String(doors[i].last_trigger_time) + ",";
        json += "\"action\":\"unlock\"";
        json += "}";
    }
...
String DataTransmitter::getNightModeJSON() {
    ...
    json += "\"movement_detected\":" + String((getCurrentTimestamp() - last_movement_time) < 5000 ? "true" : "false") + ",";
    json += "\"last_movement\":" + String(last_movement_time);
    json += "}";
    return json;
}
...
void DataTransmitter::addLog(const String& level, const String& message) {
    ...
    log_entries[log_index].timestamp = getCurrentTimestamp();
...
// 设置基准时间戳
void DataTransmitter::setCurrentTimestamp(unsigned long timestamp) {
    baseTimestamp = timestamp;
    syncTimeMillis = millis();  // 记录同步时刻
}

// 获取当前时间戳
unsigned long DataTransmitter::getCurrentTimestamp() {
    if (baseTimestamp > 0) {
        // 计算已经过去了多少秒
        unsigned long elapsed = (millis() - syncTimeMillis) / 1000;
