...
public:
    ...
    bool isWristRaised() {
        // 检测抬手：Z轴加速度变化 + 手臂抬起角度
        float accelZ = current_data.accel_z;
        float magnitude = getAccelerationMagnitude();
        
        // 条件1：Z轴加速度突变（抬手动作）
        bool motionDetected = abs(accelZ - lastAccelZ) > 0.5;
        
        // 条件2：手臂抬起（Z轴接近0，表示手表平放）
        bool raised = abs(accelZ) < 0.3;
        
        lastAccelZ = accelZ;
        
        return motionDetected && raised;
    }
