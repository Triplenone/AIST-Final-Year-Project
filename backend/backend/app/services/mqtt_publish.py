...
def publish_alert_downlink(
    ...
    *,
    ...
    """Publish FlyCare SOS/Fall control downlink to smartwatch/{device_id}/alert."""
    device_id = str(device_id or "").strip()
    if not device_id:
        raise ValueError("device_id is required for FlyCare alert downlink")
    return publish_json(
        build_flycare_alert_topic(device_id),
        dict(alert_payload),
        qos=qos,
        retain=retain,
    )
