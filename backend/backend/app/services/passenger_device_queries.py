"""Resolve devices linked to passenger users for MongoDB / vitals bridging."""
...
def devices_by_passenger_user_ids(db: Session, user_ids: List[int]) -> Dict[int, List[Device]]:
    """
    For each passenger user_id, return distinct Device rows linked by either:
    - device.elderly_user_id == user (legacy DB column), or
    - user_status.user_id == user (uses user_status.device_id so binding still works
      when the legacy device column was not set on the device row).
    """
    if not user_ids:
        return {}
    ...
    try:
        ...
        print(f"Warning: user_status linkage skipped in devices_by_passenger_user_ids: {exc}")
        rows = []
