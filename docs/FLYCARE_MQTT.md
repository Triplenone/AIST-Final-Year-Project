...
2. For PWR physical proof, observe the watch while pressing PWR once: the expected final-demo behavior is screen off. Hold PWR for 3 seconds to verify the long-press screen toggle is not broken. The source-level audit confirms the active `buttonTask`, but final physical completion still needs human visual confirmation because the backend does not receive a PWR event.
