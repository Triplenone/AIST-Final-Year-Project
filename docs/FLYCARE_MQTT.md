...
2. For PWR physical proof, observe the watch while pressing PWR once: the expected final-demo behavior is screen off. Hold PWR for 3 seconds to verify the long-press screen toggle is not broken. The source-level audit confirms the active `buttonTask` intent, but final physical completion still needs human visual confirmation because the backend does not receive a PWR event. If no `[Button] PWR...` serial line appears while pressing the hardware button, check the PWR active-level ambiguity first: `pin_config.h`/`ButtonManager.h` describe PWR as high-active, while the active `buttonTask` currently reads a LOW press.
